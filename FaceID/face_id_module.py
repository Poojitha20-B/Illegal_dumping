...
class FaceIDModule:
    ...
    def process(
        ...
        if match:
            ...
            challan_id = self._pm.create_violation(
                plate_number              = None,
                evidence_plate_image_path = evidence_path if evidence_path else face_path,
                location                  = location,
    ...
    def _override_owner(self, challan_id: str, match: dict) -> None:
        ...
        try:
            with sqlite3.connect(db_path) as conn:
                conn.execute("""
                    ...
                    SET owner_name   = ?,
                        email        = ?,
                        phone_number = ?,
                        notes        = COALESCE(notes, '') || ' | identified_by=FaceID'
                    WHERE challan_id = ?
            ...
            logger.warning("[FaceID] Could not update owner in DB: %s", e)